...
auto CategoryTable::find(const std::uint64_t cat_id) const -> std::optional<CategoryRow> {
...
auto CategoryTable::find(const std::string_view category_name) const -> std::optional<CategoryRow> {
